/* Create Sufficient prevalence*/

/* Prepare numerator for Sufficient prevalence */
...
FROM (
    ...
    ON a.subject_id = b.person_id
      ...
      AND DATEDIFF(day, a.cohort_start_date, acalendar_end_date) >= @nDays
)
GROUP BY cohort_definition_id, calendar_year, gender_concept_id, {demographic_strata} age
;

/* Prepare sufficient Denominator*/
...
FROM (
    SELECT *
    FROM #obsPop
    WHERE DATEDIFF(day, calendar_start_date, observation_period_end_date) >= @nDays
)
